...
inputX = np.random.rand(10000,300)-0.5
#inputX[:,6]=inputX[:,3]
#inputX[:,7]=inputX[:,3]
#inputX[:,8]=inputX[:,3]
#inputX[:,9]=inputX[:,3]
noise = np.random.normal(0, 0.05, inputX.shape) #inputX[:,0] *4++1.5*inputX[:,1]#
outputY = inputX[:,0] *inputX[:,0] *4 + 2*inputX[:,1]-6*inputX[:,2]+2*inputX[:,3]-inputX[:,4]+1.5*inputX[:,5]+10 #+ noiseinputX[:,0] * 4 + 1
outputY =np.reshape(outputY,[-1,1])
learning_rate=0.000025
weight1 = tf.Variable(tf.truncated_normal([300, 800], stddev=0.1,dtype=tf.float64))
bias1 = tf.Variable(tf.truncated_normal([800], stddev=0.1,dtype=tf.float64))#inputX.shape[1],
x1 = tf.placeholder(tf.float64, [None, 300])
'''
...
weight2 = tf.Variable(tf.truncated_normal([800, 300], stddev=0.1,dtype=tf.float64))
bias2 = tf.Variable(tf.truncated_normal([300], stddev=0.1,dtype=tf.float64))#inputX.shape[1],
###
wwp = tf.Variable(tf.truncated_normal([300, 300], stddev=0.1,dtype=tf.float64))
###
...
##########################################3
weightin1 = tf.Variable(tf.truncated_normal([300, 50], stddev=0.1,dtype=tf.float64))
biasin1 = tf.Variable(tf.truncated_normal([50], stddev=0.1,dtype=tf.float64))#inputX.shape[1]
yin1 = tf.matmul(y2_, weightin1) + biasin1
yin1=tf.nn.relu(yin1)
weightin2 = tf.Variable(tf.truncated_normal([50, 1], stddev=0.1,dtype=tf.float64))
biasin2 = tf.Variable(tf.truncated_normal([1], stddev=0.1,dtype=tf.float64))#inputX.shape[1]

wwpin = tf.Variable(tf.truncated_normal([300, 1], stddev=0.1,dtype=tf.float64))
...
x1er = tf.placeholder(tf.float64, [None, 300])
weightin1er = tf.Variable(tf.truncated_normal([300, 50], stddev=0.1,dtype=tf.float64))
biasin1er = tf.Variable(tf.truncated_normal([50], stddev=0.1,dtype=tf.float64))#inputX.shape[1]
yin1er = tf.matmul(x1er, weightin1er) + biasin1er
yin1er=tf.nn.relu(yin1er)
weightin2er = tf.Variable(tf.truncated_normal([50, 1], stddev=0.1,dtype=tf.float64))
biasin2er = tf.Variable(tf.truncated_normal([1], stddev=0.1,dtype=tf.float64))#inputX.shape[1]

wwper = tf.Variable(tf.truncated_normal([300, 1], stddev=0.1,dtype=tf.float64))
#biasin3 = tf.Variable(np.random.rand(50)/100)#inputX.shape[1]
...
yy = tf.placeholder(tf.float64, [None, 300])
...
'''
gr1=[wwp]
gr2=[weight1, bias1,weight2, bias2, weightin1, biasin1, weightin2, biasin2]#,weight3, bias3
gr3=[ wwpin] 
...
x_data = np.random.rand(1000,300)-0.5
#x_data[:,6]=x_data[:,3]
#x_data[:,7]=x_data[:,3]
#x_data[:,8]=x_data[:,3]
#x_data[:,9]=x_data[:,3]
...
for i in range(500):
    ...
    list2=random.sample(list1,10000)
    for j in range(0,100):
      loss_,_=sess.run([loss,train2], feed_dict={x1: inputXT[list2[j*100:(j+1)*100],:], y: outputb[list2[j*100:(j+1)*100],:]})
