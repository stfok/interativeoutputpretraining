...
weight2 = tf.Variable(tf.truncated_normal([18, 3], stddev=0.1,dtype=tf.float64))
bias2 =  tf.Variable(tf.truncated_normal([3], stddev=0.1,dtype=tf.float64))



#biasin3 = tf.Variable(np.random.rand(50)/100)#inputX.shape[1]
yin2 = tf.matmul(y1_, weight2) + bias2
...
yin3=yin2#tf.matmul(y10_, wwpin)+yin2#biaswwpin
...
train_op22 = tf.train.GradientDescentOptimizer(0.00).minimize(loss, var_list=gr1)
train2=tf.group( train_op21, train_op22) #

train_op21z = tf.train.GradientDescentOptimizer(0.0025).minimize(loss, var_list=gr1)
train_op22z = tf.train.GradientDescentOptimizer(0.0025).minimize(loss, var_list=gr2)
