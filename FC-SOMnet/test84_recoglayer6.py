...
weight1 = tf.Variable(tf.truncated_normal([6, 12], stddev=0.1,dtype=tf.float64), name='weight1')
...
##########################################3
weightin1 = tf.Variable(tf.truncated_normal([12, 6], stddev=0.1,dtype=tf.float64), name='weightin1')
biasin1 = tf.Variable(tf.truncated_normal([6], stddev=0.1,dtype=tf.float64))#inputX.shape[1]
yin1_ = tf.matmul(y1_, weightin1) + biasin1
yin1=tf.nn.relu(yin1_)
weightin2 = tf.Variable(tf.truncated_normal([6, 6], stddev=0.1,dtype=tf.float64), name='weightin2')
biasin2 = tf.Variable(tf.truncated_normal([6], stddev=0.1,dtype=tf.float64))#inputX.shape[1]

wwpin = tf.Variable(tf.truncated_normal([12, 3], stddev=0.1,dtype=tf.float64), name='weightwwpin')
...
yin20=tf.nn.relu(yin2)#+tf.matmul(y1_, wwpin)

weightin3 = tf.Variable(tf.truncated_normal([6, 3], stddev=0.1,dtype=tf.float64), name='weightin3')
...
yin3=tf.matmul(y1_, wwpin)+yin22#biasin3##+#biaswwpin
...
base_loss = tf.reduce_mean(tf.reduce_sum(tf.square((yin3 - y)), reduction_indices=[1]))
...
l2_reg = tf.constant(0.0, dtype=tf.float64)
for var in tf.trainable_variables():
    print(var.name)
    if 'weight' in var.name or 'kernel' in var.name:  # 只正则化权重矩阵
        l2_reg += tf.nn.l2_loss(var)  # L2正则项 (sum(weights^2)/2)

# 将L2正则项加到主损失函数中
loss = base_loss + 0.0 * l2_reg
...
'''
gr1=[wwpin] #wwp
gr2=[weightin1,  weightin2, weightin3, biasin2, biasin3, biasin1]#,weight2, bias2
gr3=[weight1, bias1] 
