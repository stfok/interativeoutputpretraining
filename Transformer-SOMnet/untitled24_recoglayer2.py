...
def create_vit_block(input_tensor, name):
    ...
    x = layers.MultiHeadAttention(num_heads=16, key_dim=256, name=name + '_attention')(x, x)
    x = layers.Dropout(0.1)(x)
    x = x+input_tensor
    x2 = layers.LayerNormalization(epsilon=1e-6, name=name + 'z'+'_norm2')(x)
    x_ffn = layers.Dense(256, kernel_regularizer=tf.keras.regularizers.l2(0.00001), activation='relu',  name=name + 'z'+'_ffn_1')(x2)
    
    x_ffn = layers.Dense(256, kernel_regularizer=tf.keras.regularizers.l2(0.00001), activation='relu', name=name + 'z'+'_ffn_2')(x_ffn)
    x_ffn = layers.Dropout(0.1)(x_ffn)
    x_ffn = layers.Dense(256, kernel_regularizer=tf.keras.regularizers.l2(0.00001), name=name +'z'+ '_ffn_3')(x_ffn)
    output = x + x_ffn
...
def shortcut_block(inputs, filters, block_num):
    ...
    x = layers.Conv2D(filters,kernel_regularizer=tf.keras.regularizers.l2(0.00001), kernel_size=(3, 3), padding='same', name=f'block{block_num}_conv1')(x)

    x = layers.BatchNormalization(name=f'block{block_num}_bn2')(x)
    x = layers.ReLU(name=f'block{block_num}_relu2')(x)
    x = layers.Conv2D(filters,kernel_regularizer=tf.keras.regularizers.l2(0.00001), kernel_size=(3, 3), padding='same', name=f'block{block_num}_conv2')(x)
...
def fc_shortcut_block(inputs, units, block_num):
    x = layers.BatchNormalization(name=f'fc_blockz{block_num}_bn1')(inputs)
    x = layers.ReLU(name=f'fc_blockz{block_num}_relu1')(x)
    x = layers.Dense(units, kernel_regularizer=tf.keras.regularizers.l2(0.00001),name=f'fc_blockz{block_num}_dense1')(x)
    ...
    x = layers.Dense(100, kernel_regularizer=tf.keras.regularizers.l2(0.00001), name=f'fc_blockz{block_num}_dense2')(x)
    # Shortcut connection
    #fc_shortcut = layers.Dense(units, name=f'fc_blockz{block_num}_shortcut')(inputs)
    fc_shortcut = layers.Dense(100, kernel_regularizer=tf.keras.regularizers.l2(0.00001), name=f'fcc_blockz{block_num}_shortcut')(inputs)
    return layers.add([x, fc_shortcut], name=f'fc_blockz{block_num}_output')
...
def create_vit_model(input_shape, patch_size):
    ...
    # 嵌入层
    patch_embedding = layers.Dense(256, kernel_regularizer=tf.keras.regularizers.l2(0.00001), name='patch_embedding')(patches)
    pos_embedding = position_embedding(num_patches, 256)
 ...
 for block in (6,):
    ...
    if block<6 :
      for j in range(1):
        ...
        steps_per_epoch = len(x_train) // batch_size
        #model0.fit(x_train, y_train_zeros, epochs=5, batch_size=32, steps_per_epoch=steps_per_epoch)  # 输出为零
        #model0.fit(datagen.flow(x_train, y_train_zeros, batch_size=32), epochs=20, steps_per_epoch=steps_per_epoch)  # 输出为零
